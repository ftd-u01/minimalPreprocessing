...
if [[ ! -f ${outputDir}/gaussianNoise.nii.gz ]]; then
  ${smtDir}/gaussianfit --mask $mask $allB0 ${outputDir}/gaussianB0_{}.nii.gz
fi

echo "
--- `date`   Running microscopic DT fit ---"

${smtDir}/fitmicrodt --bvals $bvals --bvecs $bvecs --graddev $gradDev --mask $mask $data ${outputDir}/mdtGaussian_{}.nii.gz

echo "
--- `date`   Running microscopic multi-compartment fit ---"

${smtDir}/fitmcmicro --bvals $bvals --bvecs $bvecs --graddev $gradDev --mask $mask $data ${outputDir}/mmcGaussian_{}.nii.gz
