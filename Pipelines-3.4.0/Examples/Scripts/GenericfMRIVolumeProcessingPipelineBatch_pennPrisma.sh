...
for Subject in $Subjlist ; do
  ...
  # https://www.mail-archive.com/hcp-users@humanconnectome.org/msg00925.html
  #
  # Note y- and not -y
  #
  #
  PhaseEncodinglist=() #x for RL, x- for LR, y for PA, y- for AP
  ...
  for seriesID in $fMRISeriesTypes; do
    if [[ -d "${StudyFolder}/${Subject}/unprocessed/3T/${seriesID}_PA" ]]; then
      Tasklist[${#Tasklist[@]}]="${seriesID}_PA"
      PhaseEncodinglist[${#PhaseEncodinglist[@]}]="y"
    fi

    if [[ -d "${StudyFolder}/${Subject}/unprocessed/3T/${seriesID}_AP" ]]; then
      Tasklist[${#Tasklist[@]}]="${seriesID}_AP"
      PhaseEncodinglist[${#PhaseEncodinglist[@]}]="y-"
    fi
  done 
  ...
  for (( i = 0; i < ${#Tasklist[@]}; i++ )); do 
    ...
    echo "  fMRI Volume processing ${fMRIName} with unwarp dir $UnwarpDir"
    ...
    SpinEchoPhaseEncodeNegative="${StudyFolder}/${Subject}/unprocessed/3T/${fMRIName}/${Subject}_3T_SpinEchoFieldMap_AP.nii.gz" #For the spin echo field map volume with a negative phase encoding direction (LR in HCP data, AP in 7T HCP data), set to NONE if using regular FIELDMAP
    SpinEchoPhaseEncodePositive="${StudyFolder}/${Subject}/unprocessed/3T/${fMRIName}/${Subject}_3T_SpinEchoFieldMap_PA.nii.gz" #For the spin echo field map volume with a positive phase encoding direction (RL in HCP data, PA in 7T HCP data), set to NONE if using regular FIELDMAP
    ...
    GradientDistortionCoeffs="/data/jet/PUBLIC/gradient_coefficients/Siemens/coeff_AS82_Prisma.grad" # SEt to NONE to skip gradient distortion correction
