...
# no ap/pa for asl
  ...
  for (( i = 0; i < ${#Tasklist[@]}; i++ )); do 
   ...
   # UnwarpDir=${PhaseEncodinglist[i]}
	UnwarpDir="NONE"
    echo "  pCASL Volume processing ${fMRIName} " # with unwarp dir $UnwarpDir
...
#    SpinEchoPhaseEncodePositive="NONE" #For the spin echo field map volume with a positive phase encoding direction (RL in HCP data, PA in 7T HCP data), set to NONE if using regular FIELDMAP
    MagnitudeInputName="${StudyFolder}/${Subject}/unprocessed/3T/${fMRIName}/${Subject}_3T_SPIRAL_V20_HCP_M0.nii.gz" #Expects 4D Magnitude volume with two 3D timepoints, set to NONE if using TOPUP
...
#    DeltaTE="2.46" #2.46ms for 3T, 1.02ms for 7T, set to NONE if using TOPUP
    ...
    if [ -n "${command_line_specified_run_local}" ] ; then
        echo "About to run ${HCPPIPEDIR}/pCASLVolume/GenericpCASLVolumeProcessingPipeline.sh"
        queuing_command=""
    else
        echo "About to use fsl_sub to queue or run ${HCPPIPEDIR}/pCASLVolume/GenericpCASLVolumeProcessingPipeline.sh"
        queuing_command="${FSLDIR}/bin/fsl_sub ${QUEUE}"
    fi

    ${queuing_command} ${HCPPIPEDIR}/pCASLVolume/GenericpCASLVolumeProcessingPipeline.sh \
      --path=$StudyFolder \
      --subject=$Subject \
      --fmriname=$fMRIName \
