...
### Editing down -- no time series for CBF
...
# echo $Upper
### Editing down stuff ends here

### Replace stuff between Start and End above with some smoothing

fslmaths "$VolumefMRI" -mas "$WorkingDirectory"/ribbon_only.nii.gz "$WorkingDirectory"/cbf_ribbon
fslmaths "$WorkingDirectory"/cbf_ribbon -bin -s $NeighborhoodSmoothing "$WorkingDirectory"/Smooth
fslmaths "$WorkingDirectory"/cbf_ribbon -s $NeighborhoodSmoothing -div "$WorkingDirectory"/Smooth -dilD "$WorkingDirectory"/cbf_ribbon_s${NeighborhoodSmoothing}
# don't think the modulate makes sense for 3d fslmaths "$WorkingDirectory"/cov -div `fslstats "$WorkingDirectory"/cov_ribbon -M` -div "$WorkingDirectory"/cov_ribbon_norm_s$NeighborhoodSmoothing "$WorkingDirectory"/cov_norm_modulate
# fslmaths "$WorkingDirectory"/cov_norm_modulate -mas "$WorkingDirectory"/ribbon_only.nii.gz "$WorkingDirectory"/cov_norm_modulate_ribbon


fslmaths "$VolumefMRI" -bin "$WorkingDirectory"/mask
# fslmaths "$WorkingDirectory"/cov_ribbon_norm_s$NeighborhoodSmoothing -thr $Upper -bin -sub "$WorkingDirectory"/mask -mul -1 "$WorkingDirectory"/goodvoxels
# fslmaths "$WorkingDirectory"/cbf_ribbon_s${NeighborhoodSmoothing} -bin -mul "$WorkingDirectory"/mask "$WorkingDirectory"/cbf_ribbon_s${NeighborhoodSmoothing}_masked
fslmaths "$WorkingDirectory"/cbf_ribbon_s${NeighborhoodSmoothing} -bin -mul "$WorkingDirectory"/mask "$WorkingDirectory"/goodvoxels
for Hemisphere in L R ; do
