#!/bin/bash 
source /home/jphillips/.bash_profile
get_batch_options() {
    local arguments=($@)
...
StudyFolder="/data/grossman/HCP/subjectsNii/" #Location of Subject folders (named by subjectID)
Subjlist="cndsTechDev01_2015_06_16_1510" #Space delimited list of subject IDs
EnvironmentScript="/data/jet/grosspeople/HCP/hcpPipeline/Pipelines-3.4.0/Examples/Scripts/SetUpHCPPipeline.sh" #Pipeline environment script
command_line_specified_run_local="TRUE"
...
LevelOneTasksList="tfMRI_WM_AP@tfMRI_WM_PA" #Delimit runs with @ and tasks with space
LevelOneFSFsList="tfMRI_WM_AP@tfMRI_WM_PA" #Delimit runs with @ and tasks with space
LevelTwoTaskList="tfMRI_WM" #Space delimited list
LevelTwoFSFList="tfMRI_WM" #Space delimited list
  ...
  for LevelTwoTask in $LevelTwoTaskList ; do
    ...
    for Subject in $Subjlist ; do
      ...
      if [ -n "${command_line_specified_run_local}" ] ; then
          ...
          queuing_command="qsub -l h_vmem=20G,s_vmem=19G"
      else
...
#      queuing_command="qsub -l h_vmem=20G,s_vmem=19G"
#      ${queuing_command} 
	${HCPPIPEDIR}/TaskfMRIAnalysis/TaskfMRIAnalysis_jsp.sh \
        --path=$StudyFolder \
        --subject=$Subject \
